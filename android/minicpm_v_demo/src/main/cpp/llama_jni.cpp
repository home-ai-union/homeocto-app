...
Java_com_example_minicpm_1v_1demo_LlamaEngine_setImageMaxSliceNumsNative(JNIEnv * /*env*/,
    ...
    if (g_ctx_vision) {
        LOGi("%s: image_max_slice_nums set to %d (stored; live update not supported in this mtmd rev, will apply on next loadMmproj)",
...
Java_com_example_minicpm_1v_1demo_LlamaEngine_processUserPrompt(
    ...
    try {
        ...
        if (!user_prompt) {
            LOGe("%s: Failed to get user prompt string", __func__);
            return 3;
        }
        ...
        if (g_ctx_vision) {
            if (!g_ctx_vision) {
                LOGe("%s: Vision context is null!", __func__);
                return 4;
            }
            ...
            mtmd_input_chunks * chunks = mtmd_input_chunks_init();
            if (!chunks) {
                LOGe("%s: Failed to init input chunks", __func__);
                return 5;
            }
        ...
        LOGi("%s: Success, stop_generation_position=%d", __func__, stop_generation_position);
        return 0;
...
Java_com_example_minicpm_1v_1demo_LlamaEngine_generateNextToken(
    ...
    try {
        ...
        if (!g_sampler) {
            LOGe("%s: Sampler is null!", __func__);
            return nullptr;
        }
