...
import com.homeai.homeocto.HomeOctoApp
...
class HomeOctoService : Service() {

    companion object {
        private const val TAG = "HomeOctoService"
        ...
        const val PICO_TOKEN = "homeocto-android-local"

        const val ACTION_START = "com.homeai.homeocto.action.START"
        const val ACTION_STOP = "com.homeai.homeocto.action.STOP"
        ...
        fun start(context: Context, publicMode: Boolean = false) {
            val intent = Intent(context, HomeOctoService::class.java).apply {
                action = ACTION_START
        ...
        fun getWorkspacePath(context: Context): String {
            ...
            return if (canWrite) {
                ...
                val fallback = context.getExternalFilesDir(null)?.resolve("homeocto")
                    ?: File(context.filesDir, "homeocto")
                fallback.mkdirs()
        ...
        fun buildEnvironment(context: Context): Map<String, String> {
            val internalHome = File(context.filesDir, "homeocto")
            internalHome.mkdirs()
            ...
            return mapOf(
                ...
                "HOMEOCTO_HOME" to workspace.absolutePath,
                "HOMEOCTO_CONFIG" to configPath,
                "HOMEOCTO_BINARY" to gatewayBinaryPath,
    ...
    private fun testBinary(binaryFile: File) {
        ...
        try {
            ...
            if (exitCode != 0) {
                throw RuntimeException(
                    "homeocto binary test failed (exit $exitCode): $output"
                )
            }
        } catch (e: java.io.IOException) {
            throw RuntimeException(
                "Cannot execute homeocto binary at ${binaryFile.absolutePath}: ${e.message}", e
            )
        }
    }
    ...
    /**
     * 运行 `homeocto onboard` 初始化配置和工作区
     */
    private fun ensureOnboarded(binaryFile: File) {
        val picoHome = File(filesDir, "homeocto")
    ...
    /**
     * 运行 web 服务进程（libhomeocto-web.so）
    ...
    private fun runWebService() {
        ...
        val configFile = File(filesDir, "homeocto/config.json")
        val env = buildEnvironment()
        ...
        try {
            ...
            procDir.listFiles()?.forEach { pidDir ->
                ...
                try {
                    ...
                    // 检查 cmdline 是否包含 homeocto
                    val cmdlineFile = File(pidDir, "cmdline")
                    if (!cmdlineFile.canRead()) return@forEach
                    val cmdline = cmdlineFile.readText()
                    if (!cmdline.contains("homeocto")) return@forEach

                    Log.i(TAG, "Killing orphan homeocto process: PID=$pid, UID=$processUid, cmd=$cmdline")
                    android.os.Process.killProcess(pid)
                } catch (e: Exception) {
                    // 忽略无权限的进程
                }
            }
            Log.i(TAG, "Cleaned up orphan homeocto processes")
        } catch (e: Exception) {
    ...
    /**
     * 构建子进程环境变量
     * 关键：设置 HOMEOCTO_BINARY 指向 gateway 二进制，让 web 服务能找到并启动 gateway
     */
    ...
    private fun createNotification(status: String): Notification {
        ...
        val stopIntent = Intent(this, HomeOctoService::class.java).apply {
            action = ACTION_STOP
        }
        ...
        return NotificationCompat.Builder(this, HomeOctoApp.CHANNEL_ID)
            .setContentTitle("HomeOcto")
