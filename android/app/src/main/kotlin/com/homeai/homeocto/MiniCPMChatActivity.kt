...
import com.example.minicpm_v_demo.ChatAdapter
import com.example.minicpm_v_demo.ChatMessage
...
import com.example.minicpm_v_demo.VideoFrameExtractor
...
class MiniCPMChatActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(MiniCPMR.layout.activity_main)
    ...
    private fun initViews() {
        recyclerChat = findViewById(MiniCPMR.id.recycler_chat)
        etInput = findViewById(MiniCPMR.id.et_input)
        btnSend = findViewById(MiniCPMR.id.btn_send)
        btnImage = findViewById(MiniCPMR.id.btn_image)
        btnClearChat = findViewById(MiniCPMR.id.btn_clear_chat)
        btnModelManager = findViewById(MiniCPMR.id.btn_model_manager)
        btnImageSlice = findViewById(MiniCPMR.id.btn_image_slice)
        cardInputBar = findViewById(MiniCPMR.id.card_input_bar)
        appBarLayout = findViewById(MiniCPMR.id.appBarLayout)
        tvTitle = findViewById(MiniCPMR.id.tv_title)
    }
    ...
    private fun setupRecyclerView() {
        ...
        chatAdapter.setOnSuggestionClick { suggestion ->
            if (isModelReady && !isProcessingVideo) {
                ...
                Toast.makeText(this, MiniCPMR.string.toast_load_model_first, Toast.LENGTH_SHORT).show()
            } else {
                Toast.makeText(this, MiniCPMR.string.toast_wait_video, Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun setupClickListeners() {
        ...
        btnModelManager.setOnClickListener {
            startActivity(Intent(this, MiniCPMModelManagerActivity::class.java))
        }
    ...
    private fun showClearChatDialog() {
        AlertDialog.Builder(this)
            .setTitle(MiniCPMR.string.clear_chat)
            .setMessage(MiniCPMR.string.clear_chat_confirm)
            .setPositiveButton(MiniCPMR.string.confirm) { _, _ ->
                clearChat()
            }
    ...
    private fun showImageSliceDialog() {
        val view = layoutInflater.inflate(MiniCPMR.layout.dialog_image_slice, null, false)
        val slider = view.findViewById<com.google.android.material.slider.Slider>(MiniCPMR.id.slider_image_slice)
        val tvValue = view.findViewById<android.widget.TextView>(MiniCPMR.id.tv_image_slice_value)
        ...
        AlertDialog.Builder(this)
            .setTitle(MiniCPMR.string.image_slice_dialog_title)
            .setView(view)
            .setPositiveButton(android.R.string.ok) { _, _ ->
                ...
                lifecycleScope.launch {
                    ...
                    val msgRes = if (engine.isVisionSupported) {
                        MiniCPMR.string.image_slice_apply_toast
                    } else {
                        MiniCPMR.string.image_slice_pending_toast
                    }
                    Toast.makeText(this@MiniCPMChatActivity, getString(msgRes, chosen), Toast.LENGTH_SHORT).show()
                }
            }
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                withContext(Dispatchers.Main) {
                    clearChatUI()
                    Toast.makeText(this@MiniCPMChatActivity, MiniCPMR.string.clear_chat_toast, Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
                Log.e(TAG, "Error clearing context", e)
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@MiniCPMChatActivity, getString(MiniCPMR.string.toast_clear_chat_failed, e.message), Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
    ...
    private fun updateUIForModelType() {
        ...
        tvTitle.setText(if (isVision) MiniCPMR.string.app_title else MiniCPMR.string.app_title_text)
    ...
    private fun loadDefaultModel() {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                hasAutoLoaded = false
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@MiniCPMChatActivity, getString(MiniCPMR.string.toast_model_load_failed, e.message), Toast.LENGTH_LONG).show()
                }
            }
        }
    }
    ...
    private fun promptDownloadModels(ggufMissing: Boolean, mmprojMissing: Boolean) {
        val message = when {
            ggufMissing && mmprojMissing ->
                getString(MiniCPMR.string.download_prompt_all_missing)
            mmprojMissing ->
                getString(MiniCPMR.string.download_prompt_mmproj_missing)
            else ->
                getString(MiniCPMR.string.download_prompt_incomplete)
        }
        AlertDialog.Builder(this)
            .setTitle(MiniCPMR.string.download_prompt_title)
            .setMessage(message)
            .setCancelable(false)
            .setPositiveButton(MiniCPMR.string.go_download) { _, _ ->
                startActivity(Intent(this, MiniCPMModelManagerActivity::class.java))
            }
            .setNegativeButton(MiniCPMR.string.later) { _, _ ->
                Toast.makeText(
                    this,
                    MiniCPMR.string.download_prompt_hint,
                    Toast.LENGTH_LONG
                ).show()
            }
            .show()
    ...
    private fun handleSelectedMedia(uri: Uri) {
        ...
        when {
            ...
            mime.startsWith("image/") || mime.isEmpty() -> handleSelectedImage(uri)
                ...
                Toast.makeText(this, getString(MiniCPMR.string.toast_unsupported_file, mime), Toast.LENGTH_SHORT).show()
            }
        }
    }
            ...
            try {
                val imageData = contentResolver.openInputStream(uri)?.use { input ->
                    val bitmap = BitmapFactory.decodeStream(input)
                        ?: throw RuntimeException(getString(MiniCPMR.string.error_decode_image))
                    val stream = ByteArrayOutputStream()
                    bitmap.compress(Bitmap.CompressFormat.PNG, 100, stream)
                    Pair(stream.toByteArray(), bitmap)
                } ?: throw RuntimeException(getString(MiniCPMR.string.error_read_image))
                ...
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@MiniCPMChatActivity, getString(MiniCPMR.string.toast_image_failed, e.message), Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
    ...
    private fun handleSelectedVideo(uri: Uri) {
        if (!engine.isVideoUnderstandingSupported) {
            Toast.makeText(this,
                MiniCPMR.string.video_only_v46,
            ...
            try {
                    ...
                    withContext(Dispatchers.Main) {
                        ...
                        if (index >= 0) {
                            ...
                            messages[index] = cur.copy(
                                imageInfo = getString(MiniCPMR.string.video_processing_progress, info, current, total)
                            )
                            chatAdapter.submitList(messages.toList())
                        }
                    }
                ...
                withContext(Dispatchers.Main) {
                    ...
                    if (index >= 0) {
                        val cur = messages[index] as ChatMessage.UserMessage
                        messages[index] = cur.copy(
                            imageInfo = getString(MiniCPMR.string.video_preprocessing_done, info, elapsedMs / 1000.0),
                            isPrefilling = false
                        )
                        chatAdapter.submitList(messages.toList())
                    }
                ...
                withContext(Dispatchers.Main) {
                    ...
                    Toast.makeText(this@MiniCPMChatActivity, getString(MiniCPMR.string.toast_video_failed, e.message), Toast.LENGTH_LONG).show()
                }
            }
        }
    }
    ...
    private fun handleUserInput() {
        ...
        if (userMsg.isEmpty()) {
            Toast.makeText(this, MiniCPMR.string.toast_empty_input, Toast.LENGTH_SHORT).show()
            return
        }
    ...
    companion object {
        private val TAG = MiniCPMChatActivity::class.java.simpleName
    }
}
